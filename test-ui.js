...
t("押した候補にaria-pressed=trueが付く", qa("[data-obj-cand]")[0].getAttribute("aria-pressed") === "true");
if (qa("[data-obj-cand]").length > 1) {
  click(qa("[data-obj-cand]")[1]);
  t("別候補を押すと前の候補はfalseに", qa("[data-obj-cand]")[0].getAttribute("aria-pressed") === "false" && qa("[data-obj-cand]")[1].getAttribute("aria-pressed") === "true");
  click(qa("[data-obj-cand]")[0]);
}
const obEl = q("#objective");
obEl.value = obEl.value + "（改）";
obEl.dispatchEvent(new window.Event("input", { bubbles: true }));
t("自由入力で候補の選択色が解除される", qa("[data-obj-cand][aria-pressed='true']").length === 0);
click(qa("[data-obj-cand]")[0]);
t("候補を再選択できる", qa("[data-obj-cand][aria-pressed='true']").length === 1);
...
click(qa("[data-step-reset='step3']")[0]);
t("STEP3リセットで学級傾向が全解除", window.APP_STATE.classTendencies.length === 0);
t("STEP3リセットで教材・学年は残る", window.APP_STATE.grade === 2 && !!window.APP_STATE.materialId);
click(chipByText("考えはあるが文章化が難しい"));
...
click(chipByText("特に困りごとはない"));
t("「特に困りごとはない」で他が全解除", JSON.stringify(window.APP_STATE.concerns) === JSON.stringify(["特に困りごとはない"]));
click(chipByText("導入が決まらない"));
t("他を選ぶと「特に困りごとはない」が解除", window.APP_STATE.concerns.includes("導入が決まらない") && !window.APP_STATE.concerns.includes("特に困りごとはない"));
click(qa("[data-clear-multi='concerns']")[0]);
t("「選択をすべて外す」で困りごと全解除", window.APP_STATE.concerns.length === 0);
click(chipByText("中心発問が決まらない"));
...
setTimeout(() => {
  ...
  /* STEP2リセット：本時条件だけ消える */
  const back3 = qa("button").find(b => b.dataset.nav === "step5"); click(back3);
  click(qa("button").find(b => b.dataset.nav === "step4"));
  click(qa("button").find(b => b.dataset.nav === "step3"));
  click(qa("button").find(b => b.dataset.nav === "step2"));
  click(qa("[data-step-reset='step2']")[0]);
  t("STEP2リセットで本時条件が消える", window.APP_STATE.lessonStage === null && window.APP_STATE.objective === "" && window.APP_STATE.fixedElements.length === 0);
  t("STEP2リセットで教材・学年は残る", window.APP_STATE.grade === 2 && window.APP_STATE.materialId !== null);

  /* 環境の「選択をすべて外す」 */
  click(qa("button").find(b => b.dataset.nav === "step1"));
  click(qa("[data-clear-multi='environment']")[0]);
  t("環境の「選択をすべて外す」が機能", window.APP_STATE.environment.length === 0);

  /* 全体リセット：下書きは残る */
  try { window.localStorage.setItem("kokugo-jugyo-gacha-draft-v1", JSON.stringify({_savedAt:"2026-07-11", grade:2})); } catch(e) {}
  click(q("#brand-home"));
  click(q("#btn-reset-all"));
  t("全体リセットの確認ダイアログ表示", d.body.textContent.includes("最初からやり直しますか"));
  click(q("#mok"));
  const init = { grade: null, materialId: null, objective: "", mode: "three" };
  t("全体リセットでstateが初期値に戻る", Object.keys(init).every(k => JSON.stringify(window.APP_STATE[k]) === JSON.stringify(init[k])) && window.APP_STATE.plans.length === 0 && window.APP_STATE.environment.length === 1);
  t("全体リセットで保存済み下書きは残る", window.localStorage.getItem("kokugo-jugyo-gacha-draft-v1") !== null);
